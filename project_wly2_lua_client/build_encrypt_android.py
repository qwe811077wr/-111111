...
VERSION_DIR = 'versions/android'
PACKAGE_DIR = "publish/android"
PACKET_NAME = 'newwl'
...
if __name__ == '__main__':
	if (len(sys.argv) >= 2):
		...
		line = file('version_android.txt').readline()
	...
	for k in versions:
		...
		f.write(' '.join(k) + '\n')
	f.close()
	f = file('publish/android/project.manifest', 'r')
	obj = json.loads(f.read())
	f.close()
	obj['assets'] = assets
	obj['version'] = cur_version
	f = file('publish/android/project.manifest', 'w')
	f.write(json.dumps(obj, indent=4, sort_keys=True))
	f.close()
	f = file('publish/android/version.manifest', 'r')
	obj = json.loads(f.read())
	f.close()
	obj['version'] = cur_version
	f = file('publish/android/version.manifest', 'w')
	f.write(json.dumps(obj, indent=4, sort_keys=True))
	f.close()

	os.system('cp -rf publish/android/version.manifest src/')
	os.system('cp -rf publish/android/project.manifest src/')

	apks = os.listdir(os.sep.join(['publish/android', 'pack']))
	for apk in apks:
		os.unlink(os.sep.join(['publish/android', 'pack', apk]))
	if os.system('cocos compile -p android -m debug --compile-script 0 -o ' + os.sep.join(['publish/android', 'pack'])) != 0:
	...
	apk_name = apks[0]
	infile = file(os.sep.join(['publish/android', 'pack', apk_name]), 'rb')
	...
	print('build android pack finish ', cur_version)
