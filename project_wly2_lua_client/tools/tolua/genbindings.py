...
def main():
    ...
    try:
        ...
        output_dir = '%s/frameworks/runtime-src/Classes/auto' % project_root

        cmd_args = {'custom_api.ini' : ('custom_api', 'lua_custom_api_auto'),
                    }
        target = 'lua'
        ...
        for key in cmd_args.keys():
            ...
            command = '%s %s %s -s %s -t %s -o %s -n %s' % (python_bin, generator_py, cfg, 'custom_api', target, output_dir, args[1])
            print command
            _run_cmd(command)
