...
if len(sys.argv) < 2:
	print("=======222")
	CONUNTRY = ""
	VERSION_MANIFEST = "publish/android/version.manifest"
	PROJECT_MANIFEST = "publish/android/project.manifest"
	RES_PATCH = None
else:
...
VERSION_DIR = getRealPath('versions/android')
PACKAGE_DIR = getRealPath("publish/android")
PACKET_NAME = 'newwl'
...
if __name__ == '__main__':
	...
	line = file(getRealPath('version_android.txt')).readline()
	...
	for k in versions:
		if len(k) > 2:
			...
			asset['group'] = '1'
			asset['size'] = os.path.getsize(os.path.join(getRealPath('publish/android'), k[2])) *1.0/1024/1024
			assets[k[0]] = asset
		f.write(' '.join(k) + '\n')
	f.close()
	...
	print('build android hot update finish ', cur_version)
