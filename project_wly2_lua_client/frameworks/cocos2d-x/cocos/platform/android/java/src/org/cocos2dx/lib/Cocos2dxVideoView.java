...
public class Cocos2dxVideoView extends SurfaceView implements MediaPlayerControl {
    ...
    private TextView skipTxtBtn = null;
...
//                resume();
    ...
    public void setSkipButtonAttr(String attr) {
        if (skipTxtBtn == null){
            return;
        }
        Map<String, Object> object = getMapForJson(attr);
        if (object == null){
            return;
        }
        Object title = object.get("title");
        if (title != null){
            skipTxtBtn.setText(title.toString());
        }
        Object font_size = object.get("font_size");
        if (font_size != null){
            skipTxtBtn.setTextSize(Integer.parseInt(font_size.toString()));
        }
        //pos
        ...
        params.leftMargin = (int) (width * 0.84);
        params.bottomMargin = (int) (height * 0.82);
        Object pos_x = object.get("pos_x");
        if (pos_x != null){
            if (pos_x instanceof Integer) {
                params.leftMargin = (int) (width * Integer.parseInt(pos_x.toString()));
            }else if (pos_x instanceof Double){
                params.leftMargin = (int) (width * Double.parseDouble(pos_x.toString()));
            }else {
                params.leftMargin = (int) (width * Float.parseFloat(pos_x.toString()));
            }
        }
        Object pos_y = object.get("pos_y");
        if (pos_y != null){
            if (pos_y instanceof Integer) {
                params.bottomMargin = (int) (height * Integer.parseInt(pos_y.toString()));
            }else if (pos_y instanceof Double){
                params.bottomMargin = (int) (height * Double.parseDouble(pos_y.toString()));
            }else {
                params.bottomMargin = (int) (height * Float.parseFloat(pos_y.toString()));
            }
        }
        skipTxtBtn.setLayoutParams(params);
        // color
        Object color = object.get("color");
        if (color != null){
            skipTxtBtn.setTextColor(Color.parseColor(color.toString()));
        }
    }

    public void addSkipButton() {
        skipTxtBtn = new TextView(this.mCocos2dxActivity);
        Typeface typeFace = Typeface.createFromAsset(this.mCocos2dxActivity.getAssets(), "res/font/hwkt.ttf");
        skipTxtBtn.setTypeface(typeFace);
        skipTxtBtn.setTextColor(Color.argb(180, 1, 255, 255));
        skipTxtBtn.setTextSize(20);
        skipTxtBtn.setGravity(Gravity.CENTER);
        ...
        params.bottomMargin = (int) (height * 0.82);
        skipTxtBtn.setLayoutParams(params);
        ((ViewGroup) this.mCocos2dxActivity.getWindow().getDecorView()).addView(skipTxtBtn);
    }
