...
function GeneralsModule:onTabChanged(sender, stop_action)
    ...
    if self._curGeneralInfo.id ~= nil then
       --该事件虽然每次发送，不过只在tab页面刚打开时接收一次
        services:dispatchEvent({name=services.EVENT_NAMES.ON_INIT_GENERALS_INFO, data = self._curGeneralInfo})
    end
end
...
function GeneralsModule:initProtocolData()
    ...
    services:addEventListener(services.EVENT_NAMES.ON_GET_GENERAL_ATTR, handler(self, self._onUpdateGenaralInfo), 'ON_GET_GENERAL_INFO' .. tostring(self))
...
function GeneralsModule:_onGeneralInfo()
    self._generalsInfoArray = {}
    local up_list = uq.cache.generals:getUpGeneralsByType(self._selectedType)
...
function GeneralsModule:updateRoleInfo(update_power, stop_action)
    ...
    if not self._curGeneralInfo.power then
        return
    end
    if update_power or stop_action then
        uq.TimerProxy:removeTimer('updateGeneralPowerGenralUpdate')
    ...
    local power = self._power or tonumber(self._powerLabel:getString())
    local pre_power = (self._curGeneralInfo.power - power) / 20
    if update_power and pre_power > 0 then
        uq:addEffectByNode(self._nodeEffect, 900006, 1, true, cc.p(40, 30), function()
            ...
            self._txtAddPower:setString("+" .. self._curGeneralInfo.power - power)
            uq.TimerProxy:addTimer('updateGeneralPowerGenralUpdate', function()
                power = power + pre_power
                self._powerLabel:setString(math.floor(power + 0.1))
            end, 0.01, 20, 0.5)
        end)
    else
...
function GeneralsModule:initBagBox()
    ...
    if self._curPageIndex > 0 then
        self._rolePageView:scrollToPage(1)
    else
        self._rolePageView:scrollToPage(0)
    end
    if self._upState then
        network:sendPacket(Protocol.C_2_S_GENERAL_ATTR_INFO, {general_id = self._generalId})
    end
end
...
function GeneralsModule:scrollEvent()
    ...
    self:_onRefreshRed()
    self._power = self._curGeneralInfo.power
    self:updateRoleInfo(false, true)
...
function GeneralsModule:getGeneralDataByIndex(index)
    if uq.cache.generals:getGeneralDataByID(self._generalId) then --上野武将
        return self._generalsInfoArray[index]
    else
        local down_list = uq.cache.generals:getDownGeneralsByType(self._selectedType)
