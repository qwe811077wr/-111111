...
function BuildNode:addPopMenu()
    ...
    self._popMenu:setData(self._buildData, self._menuConfig.Menu, self._parentMain:getMapScene():getBgLayer():getScale())
    self._parentMain:addChild(self._popMenu, self._parentMain._itemZOrder.pop_menu)
    self:positionChange()
    self:hidePopMenu()
end
...
function BuildNode:showPopMenu()
    if self._popMenu then
        ...
        self._popMenu:setLocalZOrder(self._parentMain._itemZOrder.pop_menu_top)
    end
end
...
function BuildNode:hidePopMenu()
    ...
    if self._popMenu then
        ...
        self._popMenu:setLocalZOrder(self._parentMain._itemZOrder.pop_menu)
...
function BuildNode:positionChange(map_scale)
    ...
    if self._popMenu then
        self._popMenu:setPosition(cc.p(pos_main.x + self._buildXml.x2 * map_scale, pos_main.y + self._buildXml.y2 * map_scale))
    ...
    if self._popInfo then
        self._popInfo:setPosition(cc.p(pos_main.x + self._buildXml.x3 * map_scale, pos_main.y + self._buildXml.y3 * map_scale))
    end
    if self._resInfo then
        self._resInfo:setPosition(cc.p(pos_main.x + self._buildXml.x3 * map_scale, pos_main.y + self._buildXml.y3 * map_scale))
    end
end
...
function BuildNode:addPopHelp(pop_type)
    if not self._popInfo then
        ...
        self._parentMain:addChild(self._popInfo, self._parentMain._itemZOrder.pop_info)
    end
