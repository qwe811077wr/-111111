...
function Generals:_onGetGeneralAttr(msg)
    local data = msg.data
    if not self._allGeneralsAttr[data.general_id] then
        self._allGeneralsAttr[data.general_id] = {}
    end
    for k, v in ipairs(data.attr_list) do
        self._allGeneralsAttr[data.general_id][v.attr_type] = v.value
    end

    local info = self:getGeneralDataByID(data.general_id)
    info.power = data.power
    info.leader = self._allGeneralsAttr[data.general_id][27]
    info.attack = self._allGeneralsAttr[data.general_id][28]
    info.mental = self._allGeneralsAttr[data.general_id][29]
    info.siege  = self._allGeneralsAttr[data.general_id][35]
    info.max_soldiers = self._allGeneralsAttr[data.general_id][7]
...
function Generals:getGeneralDataByID(id)
    return id and self._allGeneralsInfo.generals_map[id] or nil
end
--得到招募武将的数量
...
function Generals:getAllGeneralCanTrain()
    local data_list = {}
    uq.log(self._allGeneralsInfo.generals_map)
    for _, item in pairs(self._allGeneralsInfo.generals_map) do
...
function Generals:_updateSuddenFlight(msg)
    ...
    services:dispatchEvent({name = services.EVENT_NAMES.ON_UPDATE_GENERAL_LEVEL, data = {pre_lvl = pre_lvl, general_id = data.generals_map}})
    services:dispatchEvent({name = services.EVENT_NAMES.ON_UPDATE_GENERALS_MODULE_RED, data = {self._GENERAL_SUB_PAGE.GENERAL_ATTRIBUTE}})
end
...
function Generals:getGeneralsModuleRedByIndex(index, id)
    ...
    elseif index == 4 then
        ...
        return self:isCouldInsightById()
    end
end
...
function Generals:isCouldInsightById(id)
    local info = self._allGeneralsInfo.generals_map[general_id]
    if not info then
        return false
    end
...
function Generals:updataQualityRed()
    ...
    --self:updateRed()
    --services:dispatchEvent({name = services.EVENT_NAMES.ON_GENERALS_QUALITY_RED})
...
function Generals:updateRed(msg)
    ...
    if not is_red then
        if self._allGeneralsInfo and self._allGeneralsInfo.generals_map and next(self._allGeneralsInfo.generals_map) ~= nil then

            for k, v in pairs(self._allGeneralsInfo.generals_map) do
                if uq.cache.formation:checkGeneralIsInFormationById(k) then
                    for _, index in pairs(array) do
                        is_red = self:getGeneralsModuleRedByIndex(index, k)
                        if is_red then
                            break
                        end
                    end
