...
TrialsTowerModule.RESOURCE_BINDING  = {
    ...
    ["btn_rank"]                    ={["varname"] = "_btnRank",["events"] = {{["event"] = "touch",["method"] = "_onBtnRank"}}},
    ["btn_autosweep"]               ={["varname"] = "_btnSweep",["events"] = {{["event"] = "touch",["method"] = "_onBtnSweep"}}},
...
function TrialsTowerModule:_onTrialsTowerLoadInfo()
    ...
    for k,v in pairs(StaticData['tower_cfg']) do
        if v.ident == info.cur_layer_id then
            self._curTowerCfgInfo = v
            break
        end
    end
...
function TrialsTowerModule:updateDialog()
    ...
    self._rewardImg:setPosition(cc.p(self._rewardPos.x * scale_x, self._rewardPos.y * scale_x))
    ...
    for k,v in pairs(info_object) do
        ...
        if v.ident == uq.cache.trials_tower.trial_info.cur_npc_id and uq.cache.trials_tower.trial_info.cur_layer_id == uq.cache.trials_tower.trial_info.reward_box_layer + 1 then
            ...
            layer["userData"] = v
            layer:addClickEventListenerWithSound(function(sender)
...
function TrialsTowerModule:initDialog()
    ...
    self._rewardImg:addClickEventListenerWithSound(function(sender)
