...
GeneralsAttribute.RESOURCE_BINDING  = {
    ...
    ["btn_level_up1"]       = {["varname"] = "_btnLevelUp1",["events"] = {{["event"] = "touch",["method"] = "onLevelUp1"}}},
    ["btn_level_up5"]       = {["varname"] = "_btnLevelUp2",["events"] = {{["event"] = "touch",["method"] = "onLevelUp2"}}},
    ["node_img"]            = {["varname"] = "_nodeImg"},
    ["Panel_2"]             = {["varname"] = "_pnlBase"},
    ["Image_9"]             = {["varname"] = "_imgInternal",["events"] = {{["event"] = "touch",["method"] = "onInternal"}}},
    ["btn_down"]            = {["varname"] = "_btnDownRole",["events"] = {{["event"] = "touch",["method"] = "onDownRole"}}},
    ["Button_17"]           = {["varname"] = "_btnAttribute",["events"] = {{["event"] = "touch",["method"] = "onShowAttribute"}}},
}
...
function GeneralsAttribute:_onLevelUpSuccess(msg)
    if self._curGeneralInfo.id ~= msg.data.genaral_id then
        return
    end
    local pre_level = msg.data.pre_level
    ...
    local info = self:calculateLvlAddAttr(self._curGeneralInfo.lvl, self._curLvl)
    if info and next(info) ~= nil then
        self:runLevelUpAction(info)
    end
    uq.cache.generals:updateSuddenFlight(msg)
    self._curLvl = self._curGeneralInfo.lvl
end
