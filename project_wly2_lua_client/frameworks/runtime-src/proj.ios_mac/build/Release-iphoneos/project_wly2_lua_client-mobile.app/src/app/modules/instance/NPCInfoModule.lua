...
NPCInfoModule.RESOURCE_BINDING = {
    ...
    ["sweep_btn"]          = {["varname"] = "_btnSweep", ["events"] = {{["event"] = "touch",["method"] = "_doSweepNPC"}}},
...
function NPCInfoModule:_fillBaseInfo()
    ...
    for k, item_str in ipairs(item_xml) do
        local items = string.split(item_str, ';')
        if tonumber(items[1]) == uq.config.constant.COST_RES_TYPE.GENERALS then
            local general_data = StaticData['general'][tonumber(items[3])]
            if general_data.camp == 0 or general_data.camp == uq.cache.role.country_id then
                table.insert(reward_result, item_str)
            end
        else
            table.insert(reward_result, item_str)
        end
    ...
    for k, item_str in ipairs(reward_result) do
        ...
        local config = string.split(item_str, ';')
