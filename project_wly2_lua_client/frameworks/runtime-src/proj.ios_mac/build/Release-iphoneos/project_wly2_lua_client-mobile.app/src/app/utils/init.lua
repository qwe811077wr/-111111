...
function uq.fadeAttr(msg, x, y, color, size, font)
    x = x or display.width / 2
    y = y or display.height / 2
    uq.ModuleManager:getInstance():show(uq.ModuleManager.ATTRIBUTE_TIP_MODULE, {moduleType = 4, msg = msg, x = x, y = y, color = color, size = size, font = font})
end
...
function uq.showItemTips(info, equip_id)
    if info.type == uq.config.constant.COST_RES_TYPE.EQUIP then
        uq.ModuleManager:getInstance():show(uq.ModuleManager.ITEM_TIPS_MODULE,{info = info, pre_equip_id = equip_id})
    elseif info.type == uq.config.constant.COST_RES_TYPE.GENERALS or info.type == uq.config.constant.COST_RES_TYPE.SPIRIT then
        uq.ModuleManager:getInstance():show(uq.ModuleManager.GENERALS_TIPS_MODULE,{general_id = info.id, _type = info.type})
    else
        uq.ModuleManager:getInstance():show(uq.ModuleManager.TOOL_TIPS_MODULE,{info = info})
    end
end
...
function uq.showRoleLevelUp()
    if #uq.cache.role.level_up_array == 0 then
        services:sendPacket({name = services.EVENT_NAMES.ON_UPDATE_ALL_GENERAL_RED, array = {uq.cache.generals._GENERAL_SUB_PAGE.GENERAL_ATTRIBUTE ,uq.cache.generals._GENERAL_SUB_PAGE.GENERAL_EQUIP}})
        return
    end
...
function uq.alphaTouchCheck(img, pt)
    ...
    local alpha = math.floor(ret / div)
    return alpha > 0
end
