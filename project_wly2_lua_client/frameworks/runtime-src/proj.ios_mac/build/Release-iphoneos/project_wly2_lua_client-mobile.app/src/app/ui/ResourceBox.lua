...
function ResourceBox:setRes(icon, event_name, init_value, format_func, add_cb, id)
    ...
    if add_cb then
        ...
        self._btnAdd:addClickEventListener(add_cb)
...
function ResourceBox.createRes(type, add_cb, id)
    local cb = nil
    local default_cb = {
    ...
    local val = uq.cache.role:getResNum(type, id)
    if add_cb then
    ...
    if type == uq.config.constant.COST_RES_TYPE.GOLDEN then
        panel:setRes('img/common/ui/' .. miniIcon, services.EVENT_NAMES.ON_CONSUME_RES_CHANGE .. type, val, nil, cb, id)
    else
        panel:setRes('img/common/ui/' .. miniIcon, services.EVENT_NAMES.ON_CONSUME_RES_CHANGE .. type, val, uq.formatResource, cb, id)
    end
    return panel
end
