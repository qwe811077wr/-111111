...
ChatMain.RESOURCE_BINDING = {
    ...
    ["node_left_bottom/Panel_1/Image_3/Button_2"]       = {["varname"] = "_btnExpress",["events"] = {{["event"] = "touch",["method"] = "onShowExpress"}}},
...
function ChatMain:init()
    ...
    self._nodeBubble:setPosition(display.center)
...
function ChatMain:adaptSize()
    ...
    self._imgFriendBg:setContentSize(cc.size(friend_bg_size.width, display.height - 60))  --70是顶部栏大小
    local friend_panel_size = self._panelFriend:getContentSize()
    self._panelFriend:setContentSize(cc.size(friend_panel_size.width, display.height - 60 - 83))  --83是底部大小
end
...
function ChatMain:ChannelChangedRefresh()
    ...
    if not self._chatListViewMap[self._curChannel] then
        ...
        self._chatListViewMap[self._curChannel]:setContentSize(cc.size(display.width - self._panelChatList:getPositionX() , display.height - 60 - 83))
