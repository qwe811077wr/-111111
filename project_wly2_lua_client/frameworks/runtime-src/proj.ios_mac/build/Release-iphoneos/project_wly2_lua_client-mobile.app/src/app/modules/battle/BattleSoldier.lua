...
function BattleSoldier:setData(obj, side, flip_x, index, dir)
    self._dir = dir
    self._template = StaticData['soldier'][obj.soldier_id] or StaticData['soldier'][1]
    self._action = self._template.action
    local prefix = string.format('animation/soldier/%s', self._template.action)
    ...
    self._animationGroup = uq.AnimationManager:getInstance():getAction('soldier', self._action)
...
function BattleSoldier:onExit()
    uq.AnimationManager:getInstance():dispose('soldier', self._action)

    local animations = uq.AnimationManager:getInstance():getAnimation('soldier', self._action)
    if animations then
