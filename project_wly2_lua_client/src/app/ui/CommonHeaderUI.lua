...
CommonHeaderUI.RESOURCE_BINDING = {
    ...
    ["Node_1"]         = {["varname"] = "_nodePlayerName"},
...
function CommonHeaderUI:onCreate()
    ...
    self._btnRule:addClickEventListenerWithSound(handler(self, self.openModuleRule))
    self._btnRule:setVisible(false)
    self._imgBg:setContentSize(cc.size(display.width, 66))
    self:runActionTop()
    self._nodePlayerName:setVisible(false)
end
...
function CommonHeaderUI:setTitleText(text)
    self._nameLabel:setString(text)
end
...
function CommonHeaderUI:setPlayerName(str)
    self._nodePlayerName:setVisible(true)
    self._txtPlayerName:setString(str)
end
