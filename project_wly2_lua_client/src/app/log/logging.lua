...
local function LOG_MSG(self, level, fmt, ...)
	...
	if f_type == 'string' then
		if select('#', ...) > 0 then
			...
			if status then
				return self:append(level, file, line, msg)
			else
				return self:append(level, file, line, "Error formatting log message: " .. msg)
			end
		else
			-- only a single string, no formating needed.
			return self:append(level, file, line, fmt)
		end
	...
	return self:append(level, file, line, logging.tostring(fmt))
end
...
-------------------------------------------------------------------------------
function logging.prepareLogMsg(pattern, dt, level, file, line, message)
	local logMsg = pattern or "%date %level %file:%line %message\n"
 	message = string.gsub(message, "%%", "%%%%")
	logMsg = string.gsub(logMsg, "%%date", dt)
	logMsg = string.gsub(logMsg, "%%level", level)
    logMsg = string.gsub(logMsg, "%%file", file)
    logMsg = string.gsub(logMsg, "%%line", line)
