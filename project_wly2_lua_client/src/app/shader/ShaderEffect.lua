...
function ShaderEffect:addOutLineEffect(node, line_size, outlint_color, texture_size)
    --变灰的
    local pProgram = cc.GLProgramCache:getInstance():getGLProgram("pOutlineProgram")
    if not pProgram then
        local file_utils = cc.FileUtils:getInstance()
        local frag_source = file_utils:getStringFromFile("app/shader/stroke.h")
        pProgram = cc.GLProgram:createWithByteArrays(self.vertDefaultSource, frag_source)
        pProgram:bindAttribLocation(cc.ATTRIBUTE_NAME_POSITION, cc.VERTEX_ATTRIB_POSITION)
        pProgram:bindAttribLocation(cc.ATTRIBUTE_NAME_COLOR, cc.VERTEX_ATTRIB_COLOR)
        ...
        pProgram:use()
        pProgram:updateUniforms()
        cc.GLProgramCache:getInstance():addGLProgram(pProgram, "pOutlineProgram")
    end
    node:setGLProgram(pProgram)
    node:getGLProgramState():setUniformFloat(4, line_size)
    node:getGLProgramState():setUniformVec3(8, cc.vec3(outlint_color.r / 255.0, outlint_color.g / 255.0, outlint_color.b / 255.0))
    node:getGLProgramState():setUniformVec2(5, cc.p(texture_size.width, texture_size.height))
    node:getGLProgramState():setUniformVec3(6, cc.vec3(1.0, 1.0, 1.0))
end
