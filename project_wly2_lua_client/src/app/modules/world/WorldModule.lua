...
function WorldModule:onCreate()
    ...
    self._miniMap:setPosition(cc.p(size.width / 2 + 20, display.height - 160 + size.height / 2))
...
function WorldModule:loadSoldier()
    ...
    uq.AnimationManager:getInstance():getAction('world_soldier', self._action1)

    local soldier2 = StaticData['world_soldier'][16]
    self._action2 = string.format('%s_%d', soldier2.action, 3)
    uq.AnimationManager:getInstance():getAction('world_soldier', self._action2)
end
...
function WorldModule:onHideUI()
    ...
    self._mainView:setNodeLeftBottom(false)
    self._mainView:setNodeRightBottom(false)
    self._mainView:setNodeRightTop(false)
    self._miniMap:setVisible(false)
    self._stateNode:setVisible(false)
    for k, v in pairs(self._buildings) do
        v.build:updateInfoState(false)
    end
end

function WorldModule:updateState()
    self._stateNode:removeAllChildren()
    local button_item1 = ccui.Button:create("img/crop/s02_00047.png", "img/crop/s02_00047.png")
    button_item1:setPressedActionEnabled(true)
    button_item1:setTouchEnabled(true)
    button_item1:addClickEventListenerWithSound(function(sender)
    ...
    local pos_x = 0
    self._stateNode:addChild(button_item1)
    pos_x = pos_x + 60
    local tab_server_time = os.date("*t", uq.cache.server_data:getServerTime())
    if tab_server_time.hour < 8 then
