...
function ExtractResult:init()
    ...
    self:setCloseBack(function()
        if not self._nodeReward:isVisible() then
            return
        end
        self:disposeSelf()
    end)
...
function ExtractResult:runEquipAction()
    ...
    self._nodeEffect0:removeAllChildren()
    uq.playSoundByID(95)
    uq:addEffectByNode(self._nodeEffect0, 900014, 1, true, nil, nil, 2)
    self._nodeEffect:runAction(cc.Sequence:create(cc.DelayTime:create(12 * self._delta), cc.CallFunc:create(function()
        uq:addEffectByNode(self._nodeEffect, 900016, 1, true, nil, nil, 2)
    end)))
    ...
    self._panelBlack:runAction(cc.Sequence:create(cc.DelayTime:create(self._delta * 13), cc.FadeTo:create(self._delta * 7, 0.8 * 255),
    cc.DelayTime:create(self._delta * 25), cc.CallFunc:create(function()
...
function ExtractResult:updateCoinState()
    if not self._coinStr or next(self._coinStr) == nil then
        return
    end
