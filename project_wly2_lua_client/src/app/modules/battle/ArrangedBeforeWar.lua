...
ArrangedBeforeWar.RESOURCE_BINDING = {
    ...
    ["Button_3"]         = {["varname"] = "_btnSwitch",["events"] = {{["event"] = "touch",["method"] = "_onSwitch",["sound_id"] = 0}}},
    ...
    ["Button_30"]        = {["varname"] = "_btnConfirm",["events"] = {{["event"] = "touch",["method"] = "onConfirm",["sound_id"] = 76}}},
    ["Button_30_0"]      = {["varname"] = "_btnNationConfirm",["events"] = {{["event"] = "touch",["method"] = "onNationConfirm"}}},
    ["Button_31"]        = {["varname"] = "_btnSet",["events"] = {{["event"] = "touch",["method"] = "onSetState",["sound_id"] = 0}}},
    ...
    ["Image_23"]         = {["varname"] = "_imgBattleDec"},
    ["node_battle_att"]  = {["varname"] = "_nodeBattleAtt"},
    ["Image_battle_att"] = {["varname"] = "_imgHide",["events"] = {{["event"] = "touch",["method"] = "onHide"}}},
}
...
function ArrangedBeforeWar:ctor(name, params)
    ...
    uq.cache.instance_war._formationInfo = {}
    self._curBattleIndex = 1
    ...
    self._fromCity = params.from_city
    self._toCity = params.to_city
    ...
    self._enemyDataTotal = params.enemy_data
    self._enemyData = {}
    if self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
        self._curBattleIndex = 1
        for k, v in ipairs(self._enemyDataTotal[self._curBattleIndex]) do
            self._enemyData[v.index] = v
        end
        for k, item in ipairs(self._enemyDataTotal) do
            local data = {
                formation_id = 1,
                general_loc     = {},
            }
            table.insert(uq.cache.instance_war._formationInfo, data)
        end
    elseif self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR then
        self._curBattleIndex = 1
        for k, v in ipairs(self._enemyDataTotal[self._curBattleIndex]) do
            self._enemyData[v.index] = v
        end
        uq.cache.instance_war._formationInfo = {formation_id = 1, general_loc = {}}
    else
        for k, v in ipairs(self._enemyDataTotal) do
            self._enemyData[v.index] = v
        end
    end
    self._foodNeed = 0
...
function ArrangedBeforeWar:initInstancWar()
    self._imgBattleDec:setVisible(self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE)
    self._nodeBattleAtt:setVisible(self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR or self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE)
    if self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR or self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
        for i = 1, #self._enemyDataTotal do
            local panel = uq.createPanelOnly('embattle.FormationItem')
            panel:setName('item' .. i)
            panel:setData(i, handler(self, self.switchBattle))
            panel:setPosition(cc.p(38, 100 - (i - 1) * 45))
            self._nodeBattleAtt:addChild(panel)
        end
        self:setBattleSelect()
        self._showBattle = true
    end
end

function ArrangedBeforeWar:onHide(event)
    if event.name ~= "ended" then
        return
    end
    if self._showBattle then
        for i = 1, #self._enemyDataTotal do
            local panel = self._nodeBattleAtt:getChildByName('item' .. i)
            panel:setVisible(false)
        end
        self._showBattle = false
        self._imgHide:setFlippedY(true)
    else
        for i = 1, #self._enemyDataTotal do
            local panel = self._nodeBattleAtt:getChildByName('item' .. i)
        ...
        self._showBattle = true
        self._imgHide:setFlippedY(false)
    end
end

function ArrangedBeforeWar:setBattleSelect()
    for i = 1, #self._enemyDataTotal do
        local panel = self._nodeBattleAtt:getChildByName('item' .. i)
        panel:setSelect(i == self._curBattleIndex)
    end
end

function ArrangedBeforeWar:switchBattle(index)
    if index == self._curBattleIndex then
        return
    end
    self:saveBattleFormation()
    self._curBattleIndex = index
    self:setBattleSelect()
    self:initEnemyData(self._enemyDataTotal[self._curBattleIndex])

    if self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR then
        -- local formation_data = uq.cache.instance_war._formationInfo
    elseif self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
        local formation_data = uq.cache.instance_war._formationInfo[self._curBattleIndex]
        self:setFormation(formation_data.formation_id)
    end
end
...
function ArrangedBeforeWar:initEnemyData(data)
    ...
    for k, v in ipairs(self._enemyList) do
        v:setData(k, self._enemyData[k], self._injureState, self._gameMode)
    end
    self:refreshRestrainState()
end
...
function ArrangedBeforeWar:init()
    ...
    self._btnConfirm:setPressedActionEnabled(true)
    ...
    self._btnSoldier:setVisible(self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR)
    self:addBgEffect()

    if self._embattleType == uq.config.constant.TYPE_EMBATTLE.ENEMY then
        self._nodeRightBottom:setVisible(false)
        self._panelSwitch:setVisible(false)
        self._nodeLeftMiddle:setVisible(false)
        self._nodeAtk:setVisible(false)
        self._nodeLeftBottom:setVisible(false)
        self._nodeLeftTop:setVisible(false)
        self._nodeRightTop:setVisible(false)
    end
end

function ArrangedBeforeWar:initRestrainNode()
    local xml_data = StaticData['formation_loc'].Formationloc
    self._arrRestrainNode = {}
    for i = 1, 3 do
        local node = self._view:getChildByName("node_kezhi" .. i)
        local img_ke = node:getChildByName("img_ke")
        local img_beike = node:getChildByName("img_beike")
        local pos_x = (xml_data[3 * i + 9].x1 - xml_data[3 * i].x1) / 2 + xml_data[3 * i].x1 + 70 - display.width / 2
        local pos_y = display.height / 2 + (xml_data[3 * i + 9].y1 - xml_data[3 * i].y1) / 2 - xml_data[3 * i].y1 + 125
        node:setPosition(cc.p(pos_x, pos_y))
        node:setVisible(true)
        img_ke:setVisible(false)
        img_beike:setVisible(false)
        node:runAction(cc.RepeatForever:create(cc.Sequence:create(cc.FadeTo:create(1.5, 255 * 0.1), cc.FadeTo:create(1.5, 255))))
        table.insert(self._arrRestrainNode, {img_ke = img_ke, img_beike = img_beike})
    end
end

function ArrangedBeforeWar:refreshRestrainState(array)
    array = array or {1, 2, 3}

    if not self._curArmyData then
        return
    end
    local formation = {}
    ...
    for index, pos in pairs(array) do
        local army_data = formation[3 * pos] or formation[3 * pos - 1] or formation[3 * pos - 2]
        local enemy_data = self._enemyData[3 * pos] or self._enemyData[3 * pos - 1] or self._enemyData[3 * pos - 2]
        local is_has_state = false
        local is_ke_zhi = false
        if army_data and enemy_data then
            local general_info = uq.cache.generals:getGeneralDataByID(army_data.general_id)
            if not general_info then
                return
            end
            local soldier_xml1 = StaticData['soldier'][tonumber(general_info.battle_soldier_id)]
            local soldier_id = enemy_data.soldierId or enemy_data.soldier_id
            local soldier_xml2 = StaticData['soldier'][soldier_id]
            if not soldier_xml1 or not soldier_xml2 then
                return
            end
            local type1 = StaticData['types'].Soldier[1].Type[soldier_xml1.type]
            local type2 = StaticData['types'].Soldier[1].Type[soldier_xml2.type]
            if not type1 or not type2 then
                return
            end
            local arr_str1 = string.split(type1.ke_zhi, ',')
            local arr_str2 = string.split(type1.bei_kezhi, ',')
            for k, v in ipairs(arr_str1) do
                if tonumber(v) == soldier_xml2.type then
                    is_ke_zhi = true
                    is_has_state = true
                    break
                end
            end
            if not is_has_state then
                for k, v in ipairs(arr_str2) do
                    if tonumber(v) == soldier_xml2.type then
                        is_has_state = true
                        break
                    end
        ...
        self._arrRestrainNode[pos].img_ke:setVisible(is_has_state and is_ke_zhi)
        self._arrRestrainNode[pos].img_beike:setVisible(is_has_state and not is_ke_zhi)
    end
end

function ArrangedBeforeWar:addBgEffect()
    local bg_name = ''
    if self._bgName then
        local strs = string.split(self._bgName, '/')
        bg_name = strs[#strs]
    elseif self._npcData and self._npcData.battleBg then
        bg_name = self._npcData.battleBg
    end
...
function ArrangedBeforeWar:_onChangeBattleSoldierId(evt)
    ...
    for k, v in ipairs(self._itemList) do
        local info = v:getRoleData()
        if info and info.id == evt.data.general_id then
            v:reSetSoldierAnim()
            self:refreshRestrainState({math.ceil(v:getIndex() / 3)})
            break
        end
    end
end
...
function ArrangedBeforeWar:_saveFormation(msg)
    if self._confirmCallBack then
        self._confirmCallBack(uq.cache.instance_war._formationInfo)
    end
end
...
function ArrangedBeforeWar:saveBattleFormation()
    local tab_name = self._curArmyData.array[self._curArrayIndex]
    local formation_data = self._curArmyData[tab_name]
    local data = {
        formation_id = self._curFormation,
    ...
    data.from_id = self._fromCity
    data.to_city = self._toCity

    if self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
        uq.cache.instance_war._formationInfo[self._curBattleIndex] = data
    elseif self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR then
        uq.cache.instance_war._formationInfo = data
    end
end
...
function ArrangedBeforeWar:_onSoldier(event)
    ...
    local panel = uq.ModuleManager:getInstance():show(uq.ModuleManager.INSTANCE_WAR_DRAFT, {moduleType = uq.ModuleManager.SHOW_TYPE_REPLACE})
    panel:setData(self._fromCity)
end
    ...
    if not cell then
        ...
        for i = 1, 4 do
            ...
            if info then
                ...
                role_card:setData(index, self._curPage, info, state, self._gameMode)
                if state then
...
function ArrangedBeforeWar:upRole(battlePosisionItem, role_data, pos)
    battlePosisionItem:setData(self._curFormation, role_data.id, nil, self._gameMode)
    ...
    self:refreshRestrainState({math.ceil(pos / 3)})
end
...
function ArrangedBeforeWar:checkGeneral(id)
    ...
    if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
        data = uq.cache.instance_war:getGeneralData(id)
    else
        data = uq.cache.generals:getGeneralDataByID(id)
    end
...
function ArrangedBeforeWar:filterDownGenrealData()
    ...
    self._siftListDown = {}
    local all_general_data = nil
    if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
        if self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
            --已经存在别的阵型中的武将 不能继续上阵
            local generals = uq.cache.instance_war:getCityGeneral(self._fromCity)
            local formations = {}
            all_general_data = {}
            for k, item in pairs(uq.cache.instance_war._formationInfo) do
                if k ~= self._curBattleIndex then
                    for j, data in pairs(item.general_loc) do
                        formations[data.general_id] = true
                    end
                end
            end
            for k, item in ipairs(generals) do
                if not formations[item.id] then
                    table.insert(all_general_data, item)
                end
            end
        else
            all_general_data = uq.cache.instance_war:getCityGeneral(self._fromCity)
        end
    else
        all_general_data = uq.cache.generals:getAllGeneralData()
    end

    if not all_general_data or table.nums(all_general_data) == 0 then
        self._tableView:reloadData()
        return
    end
    ...
    if next(self._SoldierArray) ~= nil then
        ...
        for k, v in pairs(all_general_data) do
            local data = nil
            if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
                data = uq.cache.instance_war:getGeneralData(v.id)
            else
                data = uq.cache.generals:getGeneralDataByID(v.id)
            end
...
function ArrangedBeforeWar:setFormation(index)
    ...
    if self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
        ...
        local tab_name = self._curArmyData.array[self._curArrayIndex]
        self._curArmyData[tab_name] = uq.cache.instance_war._formationInfo[self._curBattleIndex].general_loc
    ...
    elseif self._embattleType == uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR then
        self._curArmyData = self:getFormationDataById(index)
        local tab_name = self._curArmyData.array[self._curArrayIndex]
        self._curArmyData[tab_name] = uq.cache.instance_war._formationInfo.general_loc
        if self._curArmyData then
            for k, v in ipairs(self._curArmyData[tab_name]) do
                formation[v.index] = v.general_id
            end
    ...
    for k, item in ipairs(self._itemList) do
        item:setData(index, formation[k], self._injureState, self._gameMode)
    end
    self:refreshRestrainState()
    self:saveBattleFormation()
    self:filterDownGenrealData()
...
function ArrangedBeforeWar:setEmbattleInfo(formation_index)
    ...
    for k, general_id in ipairs(self._allArrayGeneral) do
        ...
        if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
            general_data = uq.cache.instance_war:getGeneralData(general_id)
        else
            general_data = uq.cache.generals:getGeneralDataByID(general_id)
        end
...
function ArrangedBeforeWar:onConfirm(event)
    ...
    --战役防御不进行武将数量判断
    if general_num <= 0 and self._embattleType ~= uq.config.constant.TYPE_EMBATTLE.INSTANCE_WAR_BATTLE then
    ...
    if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
        for j, v in pairs(data.array) do
            for k, item in pairs(data[v]) do
                local general_data = uq.cache.instance_war:getGeneralData(item.general_id)
                if general_data.current_soldiers == 0 then
                    uq.fadeInfo('武将兵力为0，不可出战')
                    return
                end
            end
        end
        if not uq.cache.instance_war:checkRes(uq.config.constant.COST_RES_TYPE.FOOD, self._foodNeed) then
    ...
    for j, v in pairs(data.array) do
        for k, item in pairs(data[v]) do
            local general_data = nil
            if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
                general_data = uq.cache.instance_war:getGeneralData(item.general_id)
            else
                general_data = uq.cache.generals:getGeneralDataByID(item.general_id)
            end
...
function ArrangedBeforeWar:setMoveNodeData(role_data, roleType, from_id)
    ...
    self._moveItem:setData(self._curFormation, role_data.id, self._injureState, self._gameMode)
    self._moveItem:setVisible(false)
    self._moveCard:setVisible(false)
    self._moveCard:setRoleType(roleType)
    self._moveCard:setRoleData(role_data, nil, self._gameMode)
...
function ArrangedBeforeWar:switchRole(embattle_item, role_data, old_data)
    ...
    if old_item then
        ...
        self:refreshRestrainState({math.ceil(embattle_item:getIndex() / 3), math.ceil(old_item:getIndex() / 3)})
    else
        self:refreshRestrainState({math.ceil(embattle_item:getIndex() / 3)})
    end
    self:filterDownGenrealData()
    self:setEmbattleInfo(self._curFormation)
end
...
function ArrangedBeforeWar:refreshFood()
    if self._npcData and self._npcData.battleCoef then
        ...
        if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
            if uq.cache.instance_war:checkRes(uq.config.constant.COST_RES_TYPE.FOOD, math.ceil(food)) then
