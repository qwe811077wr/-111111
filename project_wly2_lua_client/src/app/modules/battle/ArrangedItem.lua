...
function ArrangedItem:setData(formation_index, general_id, injure_state, game_mode)
    ...
    if general_id then
        self._roleDatas = nil
        if game_mode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
            self._roleDatas = uq.cache.instance_war:getGeneralData(general_id)
        else
...
function ArrangedItem:soldierSupply()
    self:refreshSoldier()
end
...
function ArrangedItem:refreshSoldier()
    if self._roleDatas ~= nil and self._enterWar == false then
        ...
        if self._gameMode == uq.config.constant.GAME_MODE.INSTANCE_WAR then
            cur_soldier_num = self._roleDatas.current_soldiers
        end
...
function ArrangedItem:reSetSoldierAnim()
    if not self._roleDatas then
        return
    end
...
function ArrangedItem:initSoldier(id)
    ...
    for i = 1, 9 do
        ...
        local img = node_pos:getChildByName("img")
        if img then
