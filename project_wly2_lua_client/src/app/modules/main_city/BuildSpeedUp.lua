...
function BuildSpeedUp:refreshData()
    ...
    for k, item in pairs(StaticData['material']) do
        if self._isStrategy and item.type == uq.config.constant.BAG_TYPE.STRATEGY then
            table.insert(self._dataList, item)
        elseif not self._isStrategy and item.type == uq.config.constant.BAG_TYPE.BUILD then
            table.insert(self._dataList, item)
        end
    end
end
