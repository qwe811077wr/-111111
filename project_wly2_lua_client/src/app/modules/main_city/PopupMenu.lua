...
function PopupMenu:refreshItemRed()
    for k, pop_node in ipairs(self._menuNodes) do
        ...
        elseif module_id == uq.config.constant.MODULE_ID.TAVERN_VIEW then
            pop_node:getChildByName('red'):setVisible(uq.cache.generals:isHasGeneralPoolRed())
        end
    end
end
...
function PopupMenu:refreshRed()
    ...
    elseif self._buildXml.type == uq.config.constant.BUILD_TYPE.JIUGUAN then
        self._spriteRed:setVisible(uq.cache.generals:isHasGeneralPoolRed())
    end
    self:refreshItemRed()
end
