...
一个简单的富文本 Label，用法
    ...
    txt:setText(uq.Language.text[137])
...
function _M:addCustomNode(node)
    if node then
        ...
        if anc.x ~= 0.0 or anc.y ~= 0.0 then
            local tmp = node
            local siz = tmp:getContentSize()
            siz = cc.size(siz.width*node:getScaleX() , siz.height*node:getScaleY())
            node = cc.Node:create()
...
function _M:setText(text, callback , typeForChat)
    assert(text)
            ...
            while i <= len do
                if str_byte(text, i) == P_END then  -- >
                    ...
                    if i > p then
                        ...
                        elseif str_sub(str, 1, 10) == "font color" then
                            self._textColor = c3b_parse(str_sub(str, 13, 19))
                        ...
                        elseif str_sub(str, 1, 4) == "img " or str_sub(str, 1, 4) == "img_" then
                            local imageNode = self.defaultImgCb(str_trim(str_sub(str, 4, i - 1)))
                            self:addCustomNode(imageNode)
                            if typeForChat then
                                local widgetSize = imageNode:getContentSize()
                                local textType = ccui.Text:create()
                                textType:setFontName(uq.config.TTF_FONT)
                                textType:setFontSize(17)
                                textType:setAnchorPoint(cc.p(0.5, 0.5))
                                textType:setPosition(cc.p(widgetSize.width / 2.0, widgetSize.height / 2.0))
                                textType:setPositionY(widgetSize.height / 2)
                                imageNode:addChild(textType)

                                -- if typeForChat == uq.config.constant.CHAT_TYPE.PRIVATE then
                                --  textType:setString(uq.Language.text[96])
                                --  textType:setColor(cc.c3b(255 , 106 , 232))
                                -- elseif typeForChat == uq.config.constant.CHAT_TYPE.WORLD or typeForChat == uq.config.constant.CHAT_TYPE.WORLD_CONSUME then
                                --  textType:setString(uq.Language.text[97])
                                --  textType:setColor(uq.parseColor("#00F6FF"))
                                -- elseif typeForChat == uq.config.constant.CHAT_TYPE.CROP then
                                --  textType:setString(uq.Language.text[98])
                                --  textType:setColor(cc.c3b(255 , 240 , 0))
                                -- elseif typeForChat == uq.config.constant.CHAT_TYPE.BROAD then
                                --  textType:setString(uq.Language.text[99])
                                --  textType:setColor(cc.c3b(248 , 195 , 13))
                                -- elseif typeForChat == uq.config.constant.CHAT_TYPE.TEAM then
                                --  textType:setString(uq.Language.text[100])
                                --  textType:setColor(cc.c3b(0 , 255 , 96))
                                if typeForChat >= uq.config.constant.TYPE_CHAT_COUNTRY_BG.SHU and typeForChat <= uq.config.constant.TYPE_CHAT_COUNTRY_BG.WU then
                                    textType:setString(StaticData['types'].Country[1].Type[typeForChat - 1000].shortName)
                                    textType:setColor(cc.c3b(255, 255, 255))
                                    textType:setFontSize(20)
                                    textType:setPositionY(25)
                                    --imageNode:setPosition(cc.p(16, 6))
                                end
                            end
                        elseif str_sub(str, 1, 3) == "tx " or str_sub(str, 1, 3) == "tx_" then
                            local baseName = "tx_"
                            local index = (str_trim(str_sub(str, 3, i - 1)))
                            uq.log("index ::: " , index)
                            ...
                            if index < 10 then
                                ...
                                nameIndex = tostring(index) .. "_"
                            end
                            local txCfg = {7,8,5,9,14,20,5,7,5,21,5,8,18,17,20,15,10,10,8,7,10,20}
                            local frameCache = cc.SpriteFrameCache:getInstance()
                            frameCache:addSpriteFrames("img/i/chat_express_" .. index .. ".plist")
                            local animation =cc.Animation:create()
                            local frameIndex = ""
                            for i=1 , txCfg[tonumber(index)] do
                                if i < 10 then
                                    frameIndex = "0" .. i
                                else
                                    frameIndex = tostring(i)
                                end
                                print(i .. " ==== " , (baseName..nameIndex..frameIndex))
                                local spriteFrame = frameCache:getSpriteFrame(baseName..nameIndex..frameIndex ..".png")
                            ...
                            local imageNode = cc.Sprite:createWithSpriteFrameName(baseName..nameIndex.. "01" ..".png")
                            imageNode:setScale(45/80)
                            imageNode:runAction(cc.RepeatForever:create( action ))
                            self:addCustomNode(imageNode)
...
function _M:setFontSize(fontsize)
    self._fontSizeDef = fontsize
end

function _M:setTextColor(textColor)
    self._textColorDef = textColor
end
