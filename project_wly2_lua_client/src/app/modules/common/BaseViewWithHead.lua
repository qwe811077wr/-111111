...
function BaseViewWithHead:setTitleText(id)
    self._topUI:setTitleText(id)
end
...
function BaseViewWithHead:addShowCoinGroup(show_type, mode)
    mode = mode or uq.config.constant.GAME_MODE.NORMAL
    for k, v in ipairs(show_type) do
        if type(v) == "table" then
            self._topUI:addResItem(uq.ui.ResourceBox.createRes(v.type, true, v.id, mode))
        else
            self._topUI:addResItem(uq.ui.ResourceBox.createRes(v, true, nil, mode))
        end
    end
end
...
function BaseViewWithHead:setPlayerName(str)
    self._topUI:setPlayerName(str)
end
