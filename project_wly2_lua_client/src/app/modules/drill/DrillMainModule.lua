...
DrillMainModule.RESOURCE_BINDING  = {
    ...
    ["Button_1"]                  = {["varname"] = "_btnCheck",["events"] = {{["event"] = "touch",["method"] = "_onCheckPage",["sound_id"] = 0}}},
...
function DrillMainModule:init()
    ...
    services:addEventListener(services.EVENT_NAMES.ON_SET_LOCAL_ZORDER, handler(self, self.setSelfZOrder), '_onSetZorder' .. tostring(self))
end

function DrillMainModule:setSelfZOrder(msg)
    if not msg and msg.data then
        return
    end
    self:setLocalZOrder(msg.data)
end
...
function DrillMainModule:_onCheckPage(event)
    ...
    self._subModule[self._tabIndex]:showAction()
    self:refrshBtnState()
    local sound_id = self._tabIndex == 1 and 74 or 73
...
function DrillMainModule:dispose()
    ...
    self._topUI = nil
    services:removeEventListenersByTag('_onSetZorder' .. tostring(self))
