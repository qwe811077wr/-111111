...
def convert():
    ...
    if toxml:
        xmldir = os.path.realpath(cur_dir + '/../Map/Tower')
        delfiles(xmldir)
                ...
                for mapRow in range(1,xls.sheet_by_index(0).nrows):
                    ...
                    if not now_id == next_id or next_id == "":
                        ...
                        if toxml:
                            ...
                            doc = libxml2.parseDoc('<Troop_' + now_id[0:4] + '/>')

                            for k, v in data.items():
                                if not k.startswith('node_'):
                                    print_to_xml(doc.doc.getRootElement(), k, v)
                            fname = os.path.realpath(xmldir) + "/Troop_" + now_id[0:4] + ".xml"
                            of = file(fname, 'w+')
                            with(of):
                                of.write(doc.serialize('UTF-8', 1))
