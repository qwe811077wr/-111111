...
def convert(name):
    ...
    if toxml:
        xmldir = os.path.realpath(cur_dir + '/../xml/' + name)

    if tolua:
        luadir = os.path.realpath(cur_dir + '/../lua/' + name)

    if os.path.isfile(cur_dir):
        cur_dir = os.path.dirname(cur_dir)
    data_path = os.path.realpath(cur_dir + '/../excel/' + name)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        exit('Usage python convert_excel.py DIR -xml -lua')
    convert(sys.argv[1])
